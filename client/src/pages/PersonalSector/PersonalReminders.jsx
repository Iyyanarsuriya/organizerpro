...
import { getReminders, createReminder, updateReminder, deleteReminder, getCategories, createCategory, deleteCategory } from '../../api/Reminder/personalReminder';
                            ...
                            <div className="flex border-t border-slate-700/50 divide-x divide-slate-700/50 bg-slate-800/50">
                                <button
                                    onClick={() => {
                                        ...
                                        // The notification check runs every 30s and requires (now - lastNotify >= 5min)
                                        // To snooze for 10m, we set lastNotify such that the condition becomes true in 10m.
                                        // condition: now >= lastNotify + 5m
                                        // Set lastNotify = now + 10m - 5m = now + 5m
                                ...
                                <button
                                    onClick={() => {
                                        ...
                                        // Set lastNotify = now + 60m - 5m = now + 55m
                                        setLastNotifiedTimes(prev => ({
                                            ...prev,
