...
import { FaFileAlt, FaMoneyBillWave, FaUserCheck, FaPlusCircle, FaHandHoldingUsd, FaReceipt, FaSearch, FaFilter, FaTag, FaHistory } from 'react-icons/fa';
import { formatAmount } from '../../utils/formatUtils';
import ExportButtons from '../../components/Common/ExportButtons';

const EducationSalaryCalculator = ({
    periodType,
    filterMember,
    setFilterMember,
    members,
    ...
    monthlySalary,
    setMonthlySalary,
    // Education typically doesn't use units/production, but we can keep it for flexibility or remove it.
    // Keeping for now as 'Piece Rate' for ad-hoc tasks
    unitsProduced,
    setUnitsProduced,
    ratePerUnit,
    setRatePerUnit,
    ...
    const currentAttendanceEarned = useMemo(() => {
        if (salaryMode === 'daily') {
            ...
            if (periodType === 'range' && customRange.start) {
                ...
                if (parts[0]) targetDate = new Date(parseInt(parts[0]), 0, 1);
            ...
            <div className="flex flex-col gap-6 mb-8">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div className="flex flex-col gap-1">
                        <h2 className="text-xl sm:text-2xl font-black text-slate-900 tracking-tight">Education Salary & Payouts</h2>
                        {!filterMember && <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Select a staff member</p>}
                    </div>
                ...
                <div className="bg-white p-4 rounded-3xl border border-slate-100 shadow-sm grid grid-cols-1 md:grid-cols-4 gap-3">
                    ...
                    <div className="relative group">
                        ...
                        <select value={filterRole} onChange={(e) => setFilterRole(e.target.value)} className="w-full bg-indigo-50 hover:bg-indigo-100 border border-transparent rounded-2xl py-2 md:py-3 pl-8 md:pl-10 pr-6 md:pr-10 text-[10px] md:text-xs font-black text-indigo-600 text-center outline-none focus:ring-2 focus:ring-indigo-200 transition-all cursor-pointer appearance-none uppercase tracking-wide">
                            ...
                            {[...new Set(members.map(m => m.role).filter(Boolean))].sort().map(role => (
                    ...
                    <div className="relative group">
                        ...
                        <select value={filterType} onChange={(e) => setFilterType(e.target.value)} className="w-full bg-emerald-50 hover:bg-emerald-100 border border-transparent rounded-2xl py-2 md:py-3 pl-8 md:pl-10 pr-6 md:pr-10 text-[10px] md:text-xs font-black text-emerald-600 text-center outline-none focus:ring-2 focus:ring-emerald-200 transition-all cursor-pointer appearance-none uppercase tracking-wide">
                            ...
                            <option value="employee">Teacher / Admin</option>
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="border-b border-slate-100">
                                    <th className="py-4 px-4 text-[9px] font-black text-slate-400 uppercase tracking-widest">Name</th>
                                    <th className="py-4 px-4 text-[9px] font-black text-slate-400 uppercase tracking-widest text-right">Earned</th>
                                    <th className="py-4 px-4 text-[9px] font-black text-slate-400 uppercase tracking-widest text-right">Paid</th>
                                    ...
                                    <div>
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className={`px-2 py-0.5 rounded-full text-[7px] font-black uppercase tracking-widest ${members.find(m => m.id == filterMember)?.member_type === 'employee' ? 'bg-blue-100 text-blue-600' : 'bg-amber-100 text-amber-600'}`}>
                                                {members.find(m => m.id == filterMember)?.member_type || 'staff'}
                                            </span>
                                        </div>
...
export default EducationSalaryCalculator;
