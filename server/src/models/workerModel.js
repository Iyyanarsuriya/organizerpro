...
class Worker {
    static async create(data) {
        ...
        const [result] = await db.query(
            'INSERT INTO workers (user_id, name, role, phone, email, status) VALUES (?, ?, ?, ?, ?, ?)',
    ...
    static async update(id, userId, data) {
        ...
        const [result] = await db.query(
            'UPDATE workers SET name = ?, role = ?, phone = ?, email = ?, status = ? WHERE id = ? AND user_id = ?',
    ...
    static async getActiveWorkers(userId) {
        const [rows] = await db.query(
            'SELECT * FROM workers WHERE user_id = ? AND status = "active" ORDER BY name ASC',
            [userId]
        );
        return rows;
    }
