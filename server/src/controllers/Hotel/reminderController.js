...
exports.getReminders = async (req, res) => {
    try {
        ...
        console.error("Error in Hotel getReminders:", error);
...
exports.createReminder = async (req, res) => {
    try {
        ...
        console.error("Error in Hotel createReminder:", error);
        res.status(500).json({ error: 'Internal Server Error' });
    }
};
...
exports.updateReminder = async (req, res) => {
    ...
    try {
        ...
        console.error("Error in Hotel updateReminder:", error);
        res.status(500).json({ error: 'Internal Server Error' });
    }
};
...
exports.deleteReminder = async (req, res) => {
    ...
    try {
        ...
        console.error("Error in Hotel deleteReminder:", error);
        res.status(500).json({ error: 'Internal Server Error' });
    }
};
...
exports.sendMissedAlert = async (req, res) => {
    ...
    try {
        ...
        res.json({ message: `Hotel report sent for ${date || 'today'}`, ...result });
    } catch (error) {
        console.error('Hotel missed alert error:', error);
        res.status(500).json({ error: 'Internal Server Error' });
    }
};
